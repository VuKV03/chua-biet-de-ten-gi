...
> [!IMPORTANT]
> Toàn bộ 9 Phase bên dưới **đã được triển khai đầy đủ** trong source thật của repo, `dotnet build`
> 0 Error, đã smoke-test `dotnet run` thành công. Một số đoạn code trong plan gốc dưới đây **không
> compile được** hoặc gây bug runtime với version package thực tế đã cài (Swashbuckle 10.x/Microsoft.OpenApi
> 2.x đổi API breaking, AutoMapper 16.x đổi overload DI, thiếu package/FrameworkReference ở vài project,
> hash seed mẫu sai định dạng...) nên đã được sửa trực tiếp trong source — **không sửa lại trong file
> plan này** để giữ nguyên lịch sử thiết kế. Xem **[CLAUDE.md](CLAUDE.md) → mục "Khác biệt so với
> implementation_plan.md"** để biết chính xác 9 chỗ đã sửa, sửa thế nào, và tại sao — trước khi copy
> bất kỳ đoạn code nào từ file này ra dùng lại.
